            ...
            <CardContent>
              <div className="hidden overflow-x-auto lg:block">
                <table className="min-w-[860px] w-full text-xs">
                  <thead><tr className="border-b border-border text-left text-[10px] uppercase tracking-[0.12em] text-muted-foreground"><th className="pb-2.5 pr-4">Call ID</th><th className="pb-2.5 pr-4">Agent</th><th className="pb-2.5 pr-4">Region</th><th className="pb-2.5 pr-4">Issue</th><th className="pb-2.5 pr-4">Duration</th><th className="pb-2.5 pr-4">Status</th><th className="pb-2.5">Started</th></tr></thead>
                  <tbody>{filtered.slice(0, 18).map((call) => (
                    <tr key={call.id} className="group cursor-pointer border-b border-border/70 last:border-0 hover:bg-muted/25" onClick={() => onOpen(call.id)}>
                      <td className="py-3 pr-4 font-medium tabular-nums">{call.id}</td>
                      <td className="py-3 pr-4"><p className="font-medium">{call.agent_name}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{call.agent_id}</p></td>
                      <td className="py-3 pr-4 text-muted-foreground">{call.customer_region}</td>
                      <td className="max-w-[260px] py-3 pr-4"><span className="line-clamp-2">{titleCase(call.issue_type)}</span></td>
                      <td className="py-3 pr-4 tabular-nums">{formatDuration(call.duration_seconds)}</td>
                      <td className="py-3 pr-4"><StatusBadge status={call.resolution_status} /></td>
                      <td className="py-3 text-[11px] text-muted-foreground">{call.started_at ? new Date(call.started_at).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "—"}</td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
              <div className="space-y-2 lg:hidden">
                {filtered.slice(0, 12).map((call) => <MobileDataRow key={call.id} title={call.id} subtitle={`${call.agent_name} · ${titleCase(call.issue_type)}`} status={<StatusBadge status={call.resolution_status} />} meta={[call.customer_region, formatDuration(call.duration_seconds)]} onClick={() => onOpen(call.id)} />)}
              </div>
